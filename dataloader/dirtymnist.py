...
class DirtyMNIST(VisionDataset):
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self.train:
            ...
            self.data = torch.cat((self.mnist_data.unsqueeze(1),self.amnist_data),dim=0)
            self.targets = torch.cat((self.mnist_targets,self.amnist_targets),dim=0)
        else:
