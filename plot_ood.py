...
ratio_ood = {}
for m in method:
    DIR = './res/{}_ood_mnist_{}/{}/log.json'.format(args.mode,m,args.id)
    with open(DIR) as f:
        data = json.load(f)
    ...
    for key in pool_index:
        if int(key)>0:
            ...
            ratio_clean[m].append(temp1.shape[0]/total_len)
            temp1 = np.where(temp>61000)[0]
            ratio_ood[m].append(temp1.shape[0]/total_len)
    f.close()
mode = args.mode.upper()
plt.figure(figsize=(8,12))
plt.subplot(3,1,1)
plt.title("{} OOD MNIST Active Learning Test Accuracy".format(mode))
...
for e,m in enumerate(method):
    plt.plot(ratio_clean[m],label=m,color=color[e],marker='o',markersize=3)
plt.xlabel("Query Step")
plt.ylabel("Ratio")
...
for e,m in enumerate(method):
    plt.plot(ratio_ood[m],label=m,color=color[e],marker='o',markersize=3)
plt.xlabel("Query Step")
plt.ylabel("Ratio")
plt.legend(loc='upper center', ncol=5,bbox_to_anchor=(0.5, -0.2))
plt.tight_layout()
plt.savefig("./res/{}_ood_mnist_{}.png".format(args.mode,args.id))
#plt.show()